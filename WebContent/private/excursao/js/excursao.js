...
GOTRIP.excursao.cadastrar = function() {
	...
	if (valido) {
		...
		GOTRIP.excursaoRest.addExcursao({
			...
			error : function(err) {
				bootbox.alert("Erro ao cadastrar uma nova excursão: "
						+ err.responseText);
			}
		});
	}// fecha o else
...
GOTRIP.excursao.exibirExcursoes = function(listaDeExcursoes, valorBusca) {

	var html = "";

	if (listaDeExcursoes != undefined && listaDeExcursoes.length > 0
		...
		for (var i = 0; i < listaDeExcursoes.length; i++) {
			...
			"<td>"
					...
					+ "</td>"
					+ "<td>"
					+ "<img src ='/gotrip/rest/download?file="
					+ listaDeExcursoes[i].imagem1
					+ "' height ='35' width ='40'>"
					+ "</td>"
					+ "<td>"
		...
		if (listaDeExcursoes == undefined
			...
			GOTRIP.excursaoRest.buscarExcursaoPorNome({
				...
				error : function(err) {
					bootbox.alert("Erro ao consultar os participantes: "
							+ err.responseText);
				}
			});
				...
				callback : function(result) {
					if (result) {
						...
						GOTRIP.excursaoRest.deletar({
							...
							error : function(err) {
								bootbox.alert("Erro ao deletar excursão: "
										+ err.responseText);
							}
						});
...
GOTRIP.excursao.passarId = function(id) {
	...
	GOTRIP.excursaoRest.buscarExcursaoIdParticipanteNome({
		...
		success : function(listaDeParticipantes) {
			$("body").load(
					'/gotrip/private/participante/lista_participantes.html',
					function() {
						$("#id_excursao").val(id);
						GOTRIP.participante.exibirParticipantes(
								listaDeParticipantes, "", id);
...
GOTRIP.excursao.exibirResultado = function(id) {
	...
	GOTRIP.excursaoRest.buscarExcursaoPeloId({
		...
		success : function(excur) {
			...
			$("#img1").html("");
			$("<img height ='270' width ='350' >").attr("src",
					"/gotrip/rest/download?file=" + excur.imagem1).appendTo(
					"#img1")

			$("#img2").html("");
			$("<img height ='270' width ='350' >").attr("src",
					"/gotrip/rest/download?file=" + excur.imagem2).appendTo(
					"#img2")
